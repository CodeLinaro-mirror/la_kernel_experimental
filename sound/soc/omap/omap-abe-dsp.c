...
struct abe_data {
	...
	s32 *equ[MAX_EQU];
	int equ_profile[MAX_EQU];
	struct soc_enum equalizer_enum[MAX_EQU];
	struct snd_kcontrol_new equalizer_control[MAX_EQU];
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = abe->equ_profile[eqc->reg];
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (val >= abe->hdr.num_equ)
		return -EINVAL;

	equ_params.equ_length = abe->equ_texts[eqc->reg].coeff;
	size = abe->equ_texts[eqc->reg].coeff * sizeof(s32);
	memcpy(equ_params.coef.type1, abe->equ[eqc->reg] + val * size, size);
	abe->equ_profile[eqc->reg] = val;
...
static int abe_probe(struct snd_soc_platform *platform)
{
	...
	ret = request_firmware(&fw, "omap4_abe", &pdev->dev);
	if (ret != 0) {
	...
	if (abe->hdr.firmware_size > ABE_MAX_FW_SIZE) {
			dev_err(&pdev->dev, "Firmware too large at %d bytes: %d\n",
					abe->hdr.firmware_size, ret);
			ret = -EINVAL;
			goto err_fw;
	}
	dev_dbg(&pdev->dev, "ABE firmware size %d bytes\n", abe->hdr.firmware_size);

	if (abe->hdr.coeff_size > ABE_MAX_COEFF_SIZE) {
		dev_err(&pdev->dev, "Coefficients too large at %d bytes: %d\n",
					abe->hdr.coeff_size, ret);
			ret = -EINVAL;
			goto err_fw;
	}
	dev_dbg(&pdev->dev, "ABE coefficients size %d bytes\n", abe->hdr.coeff_size);

	/* get coefficient EQU mixer strings */
	if (abe->hdr.num_equ >= MAX_EQU) {
		dev_err(&pdev->dev, "Too many equalizers got %d\n", abe->hdr.num_equ);
		ret = -EINVAL;
		goto err_fw;
	}
	...
	dev_dbg(&pdev->dev, "loaded %d equalizers\n", abe->hdr.num_equ);
	for (i = 0; i < abe->hdr.num_equ; i++) {
		dev_dbg(&pdev->dev, "equ %d: %s profiles %d\n", i,
				abe->equ_texts[i].name, abe->equ_texts[i].count);
		if (abe->equ_texts[i].count >= MAX_PROFILES) {
			dev_err(&pdev->dev, "Too many profiles got %d for equ %d\n",
