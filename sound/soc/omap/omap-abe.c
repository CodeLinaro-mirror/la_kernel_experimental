...
struct omap_abe_data {
	...
	/* BE & FE Ports */
	struct omap_abe_port *port[OMAP_ABE_MAX_PORT_ID];
};
	...
	struct snd_soc_dai *dai)
{
	...
	list_for_each_entry(dsp_params, &fe->be_clients[SNDRV_PCM_STREAM_CAPTURE], list_be) {
		struct snd_soc_pcm_runtime *be = dsp_params->be;

		dev_dbg(&fe->dev, "%s: be ID=%d\n", __func__, be->dai_link->be_id);
	...
	struct snd_soc_dai *dai)
{
	...
	list_for_each_entry(dsp_params, &fe->be_clients[SNDRV_PCM_STREAM_PLAYBACK], list_be) {
		struct snd_soc_pcm_runtime *be = dsp_params->be;
...
static void enable_be_ports(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai, int stream)
{
	...
	list_for_each_entry(dsp_params, &fe->be_clients[stream], list_be) {
		...
		dev_dbg(&fe->dev, "%s: be ID=%d stream %d\n",
				__func__, be->dai_link->be_id, stream);
		...
		case OMAP_ABE_DAI_PDM_VIB:
			omap_abe_port_enable(abe_priv->abe,
					abe_priv->port[OMAP_ABE_BE_PORT_PDM_VIB]);
			break;
		case OMAP_ABE_DAI_PDM_UL:
			omap_abe_port_enable(abe_priv->abe,
...
static void enable_fe_ports(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai, int stream)
{
	struct snd_soc_pcm_runtime *fe = substream->private_data;
	struct omap_abe_data *abe_priv = snd_soc_dai_get_drvdata(dai);

	dev_dbg(&fe->dev, "%s: fe ID=%d stream %d\n",
			__func__, fe->cpu_dai->id, stream);
...
static void disable_be_ports(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai, int stream)
{
	struct snd_soc_pcm_runtime *fe = substream->private_data;
	struct omap_abe_data *abe_priv = snd_soc_dai_get_drvdata(dai);
...
static void disable_fe_ports(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai, int stream)
{
	struct snd_soc_pcm_runtime *fe = substream->private_data;
	struct omap_abe_data *abe_priv = snd_soc_dai_get_drvdata(dai);
		...
		struct snd_soc_dai *dai, int cmd)
{
	...
	dev_dbg(&fe->dev, "%s-start: fe ID=%d stream %d\n",
			__func__, fe->cpu_dai->id, substream->stream);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
		/*   ALL BE GAINS set to mute */
		mute_be_capture(substream, dai);

		/* Enable ALL ABE BE ports */
		enable_be_ports(substream, dai, SNDRV_PCM_STREAM_CAPTURE);

		/* DAI work must be started/stopped at least 250us after ABE */
		udelay(250);

		/*  trigger ALL BE ports */
		soc_dsp_be_dai_trigger(fe, substream->stream, cmd);

		/* Enable Frontend sDMA  */
		enable_fe_ports(substream, dai, SNDRV_PCM_STREAM_CAPTURE);

		/* Restore ABE GAINS AMIC */
		unmute_be_capture(substream, dai);
		break;
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		disable_fe_ports(substream, dai, SNDRV_PCM_STREAM_CAPTURE);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
		/* Disable sDMA */
		disable_fe_ports(substream, dai, SNDRV_PCM_STREAM_CAPTURE);

		disable_be_ports(substream, dai, SNDRV_PCM_STREAM_CAPTURE);
	...
	dev_dbg(&fe->dev, "%s - finish: fe ID=%d stream %d\n",
			__func__, fe->cpu_dai->id, substream->stream);
}
		...
		struct snd_soc_dai *dai, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		/* mute splayback */
		mute_be_playback(substream, dai);

		/* enabled BE ports */
		enable_be_ports(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);
		...
		/* TODO: unmute DL1 */
		unmute_be_playback(substream, dai);

		/* Enable Frontend sDMA  */
		enable_fe_ports(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);

		/* unmute ABE_MM_DL */
		unmute_fe_port(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);
		break;
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		/* Enable Frontend sDMA  */
		enable_fe_ports(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		/* Enable Frontend sDMA  */
		disable_fe_ports(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);

		/* mute ABE_MM_DL */
		mute_fe_port(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
		...
		disable_be_ports(substream, dai, SNDRV_PCM_STREAM_PLAYBACK);
...
static struct abe tmp_abe;  /* tmp until HAL is ready */
...
static int omap_abe_dai_probe(struct snd_soc_dai *dai)
{
	...
	/* tmp - see above */
	abe_priv->abe = &tmp_abe;
	spin_lock_init(&abe_priv->abe->lock);
	INIT_LIST_HEAD(&abe_priv->abe->ports);
