...
int soc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	/* dsp DAI links dont need compat checks */
				   ...
				   int num, int dailess)
{
	...
	mutex_init(&rtd->fe_mutex);
	INIT_LIST_HEAD(&rtd->be_clients[SNDRV_PCM_STREAM_PLAYBACK]);
	INIT_LIST_HEAD(&rtd->be_clients[SNDRV_PCM_STREAM_CAPTURE]);
	INIT_LIST_HEAD(&rtd->fe_clients[SNDRV_PCM_STREAM_PLAYBACK]);
	INIT_LIST_HEAD(&rtd->fe_clients[SNDRV_PCM_STREAM_CAPTURE]);
